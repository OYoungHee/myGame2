...
ObjectID makeMos(int n)
{
    ...
    x = rand() % 1000;
    y = rand() % 1000;
    mos[n] = createObject("모기", "크기변환_모기.jpg", scene1, x, y, true);
    setTimer(mostimer[n], 10.0f);
...
int main(void)
{
    ...
    startButton = createObject("시작버튼", "start.png", scene1, 590, 70, true);
    endButton = createObject("종료버튼", "end.png", scene1, 590, 20, true);
    //allButton = createObject("총 몇마리?", "all.png", scene1, 590, 110, true);
    for (int i = 0; i < 10; i++)
        mostimer[i] = createTimer(10.0f);
